...
export function nextTick(
  ...
  return fn ? p.then(context ? fn.bind(context) : fn) : p
}
// import {
//   render,
//   nodeOps,
...
describe('api: watch', () => {
   ...
   * 测试用例 start
   */
   it('directly watching reactive object', async () => {
    const src = reactive({
      count: 0,
      count1: 0
    })
  ...
  it('watching single source: getter', async () => {
    const state = reactive({ count: 0, count1: 0 })
    watch(
      () => state.count,
      () => {
  ...
  it('cleanup registration (with source)', async () => {
    ...
    let dummy
    watch(() => count.value, (count, prevCount, onCleanup) => {
      onCleanup(cleanup)
      dummy = count
    })
